...
public class ControleSinais15 : MonoBehaviour {
...
//	public float L; //L da série de Fourier
	...
	Vector3[] positions = new Vector3[300]; //Vetor de posições
	private LineRenderer lr; //LineRenderer lr
	public Slider A1; //Slider A1, A2...
	public Slider A2;
	public Slider A3;
	public Slider A4;
	public Slider A1fase;
	...
	//A função calcula o valor de y para cada número de x(i), que começa em -12 e incrementa 0.1 até completar 300 pontos no total.
		...
		//A1.valeu é o valor retornado de cada Slider
			yvalue = (A1.value*Mathf.Sin(Mathf.PI*i + A1fase.value*Mathf.PI/180)+A2.value*Mathf.Sin(Mathf.PI*i+A2fase.value*Mathf.PI/180)+ A3.value*Mathf.Sin(Mathf.PI*i+A3fase.value*Mathf.PI/180)+(1)*A4.value*Mathf.Sin(Mathf.PI*i+A4fase.value*Mathf.PI/180));
			//Positions é um vetor de Vector3, onde cada um de seu valor consta um Vector3 com posições de i e yvalue (y=f(x))	
			positions [pos] = new Vector3 (i, 0.6f*yvalue + 2, 0.0f);
